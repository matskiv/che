...
public class ComposeEnvironmentValidatorTest {
  ...
  public void setUp() throws Exception {
    ...
    when(composeEnv.getServices()).thenReturn(singletonMap(MACHINE_NAME, service));
  ...
  public void shouldSucceedOnValidationOfValidEnvironment() throws Exception {
    ...
    when(composeEnv.getServices())
        .thenReturn(ImmutableMap.of(MACHINE_NAME, service, machine2Name, container2));
    ...
    // when
    composeEnvironmentValidator.doValidate(composeEnv);
  }
  ...
  public void shouldFailIfContainersListIsEmpty() throws Exception {
    // given
    when(composeEnv.getServices()).thenReturn(emptyMap());
  ...
  public void shouldFailIfMachineIsNotInContainersList() throws Exception {
    // given
    when(composeEnv.getMachines()).thenReturn(singletonMap(MACHINE_NAME, machineConfig));
    when(composeEnv.getServices()).thenReturn(singletonMap("machine2", service));
  ...
  public void shouldFailIfContainerNameIsInvalid(String name) throws Exception {
    // given
    when(composeEnv.getServices()).thenReturn(singletonMap(name, service));
  ...
  public void shouldPassIfMachineFromRecipeIsNotInTheListOfMachines() throws Exception {
    ...
    when(composeEnv.getMachines()).thenReturn(singletonMap(MACHINE_NAME, machineConfig));
    when(composeEnv.getServices())
        .thenReturn(ImmutableMap.of(MACHINE_NAME, service, "anotherContainer", container2));
